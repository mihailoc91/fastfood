...
public class DobavljacOut extends javax.swing.JPanel implements IZaposleni{
    ...
    public DobavljacOut() throws SQLException {
        initComponents();
        jList1.setModel(ControllerExit.setJListModel(1,4));
        jLista1=jList1;
        

        jRadioButton1.setActionCommand("1");
        jRadioButton1.setText("Dobavljaci: ");
        
        
        ControllerExit.setJTableStartLD(jTable1,4);
        jt = jTable1;
        ...
        jCheckBox1.setBounds(width, height+ControllerExit.Height61p, ControllerExit.Width10p, ControllerExit.Height4p);
    ...
    private void initComponents() {
        ...
        jCheckBox1.setText("Sortiraj po vrednosti nabavki");
        add(jCheckBox1);
        jCheckBox1.setBounds(86, 436, 229, 29);
    ...
    private void jButtonOdaberiActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonOdaberiActionPerformed
        try {
                if(jCheckBox1.isSelected()){
                    ControllerExit.setJTableDobavljaciPoVrednosti(jTable1);
                }else{
                    jTable1=ControllerExit.setJTableNabavkaMD(jTable1,ControllerExit.getSelectedFromJRButtonGroup(buttonGroup1,op), ControllerExit.getSelectedFromJRButton(buttonGroup1,jList1,null,null,null,op,4),4);
                }
         ...
         op.showMessageDialog(op.getParent(),"Niste odabrali parametre za pretragu, molimo vas odaberiti parametre i pokusajte ponovo!","Info",JOptionPane.INFORMATION_MESSAGE);
            ...
            Logger.getLogger(DobavljacOut.class.getName()).log(Level.SEVERE, null, ex);
        }catch (Exception ex){
            ex.printStackTrace();
        }
    ...
    private void jButtonIzmeniActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonIzmeniActionPerformed
        try {
            DobavljacUpdate du = new DobavljacUpdate (ControllerExit.getSelectedFromTable2(jTable1,4));
            JFrame jf = new JFrame ();
            jf.setContentPane(du);
            jf.setExtendedState(JFrame.MAXIMIZED_BOTH);
            jf.setVisible(true);
            jf.revalidate();
            zaposleni.addEventListener(du);
            zaposleni.distributeEvent();
        }catch(ArrayIndexOutOfBoundsException ex) {
            op.showMessageDialog(op.getParent(),"Niste odabrali dobavljaca koga zelite da izmenite!","Info",JOptionPane.INFORMATION_MESSAGE);
    ...
    private void jButtonIzbrisiActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonIzbrisiActionPerformed
          try {
            boolean flag = ControllerExit.deleteFromTable(jTable1,4);
            if(flag){
                
            op.showMessageDialog(op.getParent(),"Uspesno izbrisan Dobavljac!","Brisanje",JOptionPane.INFORMATION_MESSAGE);
            jList1.setModel(ControllerExit.setJListModel(1,4));
            }else{
                op.showMessageDialog(op.getParent(),"Neuspesno izbrisan Dobavljac!","Brisanje",JOptionPane.INFORMATION_MESSAGE);
            }
        } catch (SQLException ex) {
