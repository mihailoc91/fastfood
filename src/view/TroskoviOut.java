...
public class TroskoviOut extends javax.swing.JPanel implements IZaposleni{
    ...
    public TroskoviOut() throws SQLException {
        initComponents();
        jList1.setModel(ControllerExit.setJListModel(1,6));
        jLista1=jList1;
        jList2.setModel(ControllerExit.setJListModel(2,6));
        jLista2=jList2;
        
        jRadioButton1.setActionCommand("1");
        jRadioButton1.setText("Datum: ");
        jRadioButton2.setActionCommand("2");
        jRadioButton2.setText("Tip troska: ");
        

        ControllerExit.setJTableStartLD(jTable1,6);
        jt=jTable1;
        jLabelTitle.setText("Ukupan iznos: ");
        jLabelValue.setText(Trosak.ukupnaVrednostTroskova());
        jl=jLabelValue;
        ...
        jScrollPane3.setBounds(width+ControllerExit.Width29p, height+ControllerExit.Height14p, ControllerExit.Width10p, ControllerExit.Height14p);
    ...
    private void initComponents() {
        ...
        add(jRadioButton2);
        jRadioButton2.setBounds(479, 61, 137, 31);
    ...
    private void jButtonOdaberiActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonOdaberiActionPerformed
        try {
            jTable1=ControllerExit.setJTableNabavkaMD(jTable1,ControllerExit.getSelectedFromJRButtonGroup(buttonGroup1,op), ControllerExit.getSelectedFromJRButton(buttonGroup1, jList1,null,jList2,null,op,6),6);
            jLabelValue.setText(ControllerExit.ukupnaVrednost(ControllerExit.getSelectedFromJRButtonGroup(buttonGroup1,op),ControllerExit.getSelectedFromJRButton(buttonGroup1, jList1,null,jList2,null,op,6),6));
            ...
            Logger.getLogger(TroskoviOut.class.getName()).log(Level.SEVERE, null, ex);
        }catch (Exception ex){
            ex.printStackTrace();
        }
    ...
    private void jButtonIzmeniActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonIzmeniActionPerformed
        try {
            TroskoviUpdate tu = new TroskoviUpdate (ControllerExit.getSelectedFromTable2(jTable1,6));
            JFrame jf = new JFrame ();
            jf.setContentPane(tu);
            jf.setExtendedState(JFrame.MAXIMIZED_BOTH);
            jf.setVisible(true);
            jf.revalidate();
            zaposleni.addEventListener(tu);
            zaposleni.distributeEvent();
        }catch(ArrayIndexOutOfBoundsException ex) {
            op.showMessageDialog(op.getParent(),"Niste odabrali trosak koji zelite da izmenite!","Info",JOptionPane.INFORMATION_MESSAGE);
    ...
    private void jButtonIzbrisiActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonIzbrisiActionPerformed
          try {
            boolean flag = ControllerExit.deleteFromTable(jTable1,6);
            if(flag){
                
            op.showMessageDialog(op.getParent(),"Uspesno izbrisan Trosak!","Brisanje",JOptionPane.INFORMATION_MESSAGE);
            jList1.setModel(ControllerExit.setJListModel(1,6));
            jList2.setModel(ControllerExit.setJListModel(2,6));
            }else{
                op.showMessageDialog(op.getParent(),"Neuspesno izbrisan Trosak!","Brisanje",JOptionPane.INFORMATION_MESSAGE);
            }
        } catch (SQLException ex) {
